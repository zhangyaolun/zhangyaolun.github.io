  ...
  <el-card class="box-card action-add-info">
    ...
    <el-form ref="ruleForm" :model="oData" :rules="rules" label-width="150px">
      ...
      <el-form-item label="使用期限：" prop="dynamicPeriodDays" v-show="oData.dynamicPeriodFlg === 1">
        <el-input class="mr dynamicPeriodDays inlineBlock" v-model.number="oData.dynamicPeriodDays" /><span>天</span>
      </el-form-item>
      ...
      <el-form-item label="图片设置：" prop="imagePathDetail">
        ...
        <div class="img-content">
          ...
          <el-upload v-else ref="uploadimage" :show-file-list="false" :before-upload="beforeAvatarUpload" :headers="headers" :http-request="requestFile" :action="action">
            <el-button size="mini" type="primary">点击上传</el-button>
          </el-upload>
        </div>
        <span style="color: #909399;font-size: 12px;">建议尺寸400x400像素，不超过1MB，支持JPG、PNG、JPEG格式</span>
      </el-form-item>
...
<script>
import ImageItem from '../_part-image-item.vue'
import CouponList from '@/components/adminCouponList'
import { formatBackendTime } from '@/filters/index'
import { couponMixin } from "../mixin.js"

export default {
  mixins: [couponMixin],
  props: ['submitFalg', 'couponIds'],
  components: {
    ImageItem,
    CouponList
  },
  mounted () {
    let _couponData = this.$store.state.coupon.couponData
    if (_couponData) {
      this.oData = Object.assign(this.oData, _couponData)
      this.$emit('load', false)
    } else {
      this.$api.promotionCoupon.detail({ id: this.couponIds }).then(res => {
        this.oData = Object.assign(this.oData, res.data)
        this.oData.transferFlag === 0 ? this.$set(this.oData, 'giveLimitFalg', false) : this.$set(this.oData, 'giveLimitFalg', true)
        this.info()
      }).catch(error => {
  ...
  methods: {
    info () {
      if (this.oData.dynamicPeriodFlg == 0) {
        this.oData.useDateFromStr = formatBackendTime(this.oData.useDateFrom)
        this.oData.useDateToStr = formatBackendTime(this.oData.useDateTo)
      }
      if (this.oData.type != 2) {
        if (this.oData.commodityCdList && this.oData.commodityCdList.length > 0) {
          ...
          this.oData.commodityCdList.forEach((v, k) => {
            ...
            this.seachShopData(k, 1)
          })
        }
      } else {
        this.oData.csvList[this.oData.commodityFlg].falg = false
        this.oData.csvList[this.oData.commodityFlg].list = this.oData.commodityCdList
        if (this.oData.commodityFlg == 1) {
          this.oData.csvList[this.oData.commodityFlg].name = '适用商品'
        } else if (this.oData.commodityFlg == 2) {
          this.oData.csvList[this.oData.commodityFlg].name = '不适用商品'
        }
      }
      if (this.oData.shopFlg == 1) {
        this.oData.csvList[0].falg = false
        this.oData.csvList[0].name = '适用门店'
        this.oData.csvList[0].list = this.oData.shopIdList
      } else {
