      ...
      <el-form :model="bindModel" :rules="bindModel.rules" ref="bundleBindForm">
        <el-table :data="bindModel.tableData">
          <el-table-column label="绑定券id">
            <template slot-scope="scope">
              <el-form-item  :prop="'tableData.' + scope.$index + '.couponId'" :rules='bindModel.rules.couponId'>
                <el-input v-model.number="scope.row.couponId" size="mini" placeholder="绑定券id" @change="handlerCouponId" />
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column prop="couponNum" label="绑定券数量" min-width="120">
            <template slot-scope="scope">
              <el-form-item :prop="'tableData.' + scope.$index + '.couponNum'" :rules='bindModel.rules.couponNum'>
                <el-input  v-model.number="scope.row.couponNum" size="mini" placeholder="绑定券数量" />
              </el-form-item>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-form-item>
                <el-button size="mini" type="danger" @click="$emit('handleDelete', scope.$index)">删除</el-button>
              </el-form-item>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    handlerCouponId(value) {
      // if (this.tableData.some(ele => parseInt(ele.couponId) === parseInt(value))) {
      //   this.$alert("该券已经绑定！")
      //   return
      // }
