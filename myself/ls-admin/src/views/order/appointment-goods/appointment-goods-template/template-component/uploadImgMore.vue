<template>
  <section>
    <el-upload
      ...
      :limit="5"
      :file-list="info.imageBigs"
      :headers="headers"
      :on-success="bigFileSuccess" :before-upload="beforeAvatarUpload" :class="{hideUpload: info.imageBigs.length >= 5 || info.pageType === 'detail'}">
...
export default {
  ...
  methods: {
    bigFileSuccess (res, file) {
      this.info.imageBigs.push({ url: res.data })
      this.$parent.$parent.$parent.$parent.$refs.ruleForm.validateField('imageBigs')
    ...
    handleRemove (file) {
      this.info.imageBigs = this.info.imageBigs.filter(v => {
    ...
    beforeAvatarUpload (file) {
      ...
      const isSize = new Promise((resolve, reject) => {
        ...
        this.$store.dispatch('appointmentGoods/setLoadingData', false)
        this.$message.error('上传的图片尺寸为800*800')
