...
export default {
  ...
  computed: {
    ...
    imageList () {
      let imglist = []
      if (this.imageName === 'imageSmall') {
        this.info.imageSmall ? imglist = [{ url: this.info.imageSmall }] : imglist = []
      }
      return imglist
    }
  },
  methods: {
    bigFileSuccess (res, file) {
      if (this.imageName === 'imageSmall') {
        ...
        this.$parent.$parent.$parent.$parent.$refs.ruleForm.validateField('imageSmall')
      }
    ...
    handleRemove () {
      this.imageName === 'imageSmall' ? this.$set(this.info, this.imageName, '') : ''
    },
