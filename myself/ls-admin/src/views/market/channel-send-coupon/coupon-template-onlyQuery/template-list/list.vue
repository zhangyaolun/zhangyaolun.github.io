  ...
  <el-card class="list-table">
    ...
    >
      ...
      <el-table-column prop="" label="优惠券领取状态" min-width="120">
        <template slot-scope="scope">
          <router-link :to="'/backend/channelSendCoupon/templateOnlyQueryReceivelist/' + scope.row.activityKey" v-if="scope.row.status === 'PUBLISHED'">
      ...
      <el-table-column prop="" label="操作" min-width="170">
        <template slot-scope="scope">
          <template v-if="scope.row.status === 'PUBLISHED'">
            <router-link :to="'/backend/channelSendCoupon/templateOnlyQueryDetail/' + scope.row.id" class="ml10">
