  ...
  computed: {
    ...
    imageList () {
      ...
      if (this.imageName === 'merchantLogoUrl') {
        ...
        this.info.couponInfo.thumbnailUrl ? imglist = [{ url: this.info.couponInfo.thumbnailUrl }] : imglist = []
      }
      return imglist
    }
  },
  methods: {
    ...
    bigFileSuccess (res, file) {
      if (this.imageName === 'merchantLogoUrl') {
        ...
        this.$parent.$parent.$parent.$parent.$refs.ruleForm.validateField('couponInfo.thumbnailUrl')
      }
      this.$store.dispatch('payCourtesy/setLoadingData', false)
    },
    uploadUrl: function () {
