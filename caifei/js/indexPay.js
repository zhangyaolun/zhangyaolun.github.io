		...
		var sucess = function(data){
            ...
            for(var i = 0; i < oData.result.length; i++){
            	if(i == 0){
            		...
            		result +=  '<li class="detailColor item_first marginLeft" ><div class="payItem_left"><img src="'+oData.result[i].pic+'" alt="" result="'+JSON.stringify(oData.result[i]).replace(/"([^"]*)"/g, "'$1'")+'"/></div><div class="payItem_right"><div class="detailMain detailColor"><span class="main_title">'+oData.result[i].shopName+'</span><div class="mr_center clear"><span class="marght left">综合评价：<em class="emStyle">'+oData.result[i].averageScore.toFixed(1)+'</em></span><span class="right pay_num">'+oData.result[i].distance.toFixed(2)+'<em>km</em></span></div><div class="main_price clear"><span class="price_code left"><em>'+(oData.result[i].discountRate*10)+'</em>折</span><span class="price_img right" data="'+JSON.stringify(oData.result[i]).replace(/"([^"]*)"/g, "'$1'")+'" type="'+oData.result[i].averageScore.toFixed(1)+'">付&nbsp;&nbsp;款</span></div></div></div></li>'
            	}else{
            		result +=  '<li class="detailColor item_first marginright" ><div class="payItem_left"><img src="'+oData.result[i].pic+'" alt="" result="'+JSON.stringify(oData.result[i]).replace(/"([^"]*)"/g, "'$1'")+'"/></div><div class="payItem_right"><div class="detailMain detailColor"><span class="main_title">'+oData.result[i].shopName+'</span><div class="mr_center clear"><span class="marght left">综合评价：<em class="emStyle">'+oData.result[i].averageScore.toFixed(1)+'</em></span><span class="right pay_num">'+oData.result[i].distance.toFixed(2)+'<em>km</em></span></div><div class="main_price clear"><span class="price_code left"><em>'+(oData.result[i].discountRate*10)+'</em>折</span><span class="price_img right" data="'+JSON.stringify(oData.result[i]).replace(/"([^"]*)"/g, "'$1'")+'" type="'+oData.result[i].averageScore.toFixed(1)+'">付&nbsp;&nbsp;款</span></div></div></div></li>'
            	}
